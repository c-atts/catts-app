...
export default function PopularRecipesPagination({
  currentPage,
  totalPages,
}: {
  currentPage: number;
    ...
    if (startPage > 1) {
      pages.push(
        <PaginationItem key={1}>
          <PaginationLink search={{ page: 1 }} to={`/popular`}>
            1
          </PaginationLink>
        </PaginationItem>,
      );
    ...
    for (let page = startPage; page <= endPage; page++) {
      pages.push(
        <PaginationItem key={page}>
          <PaginationLink search={{ page }} to={`/popular`}>
            {page}
          </PaginationLink>
        </PaginationItem>,
      );
    ...
    if (endPage < totalPages) {
      ...
      pages.push(
        <PaginationItem key={totalPages}>
          <PaginationLink search={{ page: totalPages }} to={`/popular`}>
            {totalPages}
          </PaginationLink>
        </PaginationItem>,
      );
